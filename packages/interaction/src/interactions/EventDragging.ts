...
export class EventDragging extends Interaction {
  // TODO: rename to EventSelectingAndDragging
  ...
  handleHitUpdate = (hit: Hit | null, isFinal: boolean) => {
    ...
    if (hit) {
      ...
      if (
        ...
        if (mutation) {
          ...
          if (
            !isInteractionValid(interaction, hit.dateProfile, receivingContext)
          ) {
            isInvalid = true;
            mutation = null;
    ...
    if (!isFinal) {
      ...
      this.dragging.setMirrorIsVisible(
        !hit ||
          !(this.subjectEl.getRootNode() as ParentNode).querySelector(
            '.fc-event-mirror'
          ) // TODO: turn className into constant
      );
  ...
  handleDragEnd = (ev: PointerDragEvent) => {
    if (this.isDragging) {
      ...
      // TODO: URGEN: CHANGE: FIND ME: SOLUTION: when we drag and element fast it doesn't trigger the drop event
      receivingContext = initialContext;
      //
  ...
  displayDrag(
    nextContext: CalendarContext | null,
